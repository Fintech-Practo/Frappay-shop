...
export default function AdminRefundLedger() {
  ...
  const handleAction = async (refundId, action) => {
    try {
      ...
      if (res.success) {
        toast.success(
          `Refund ${action === "settle" ? "settled" : action === "process" ? "processing" : "failed"} successfully`,
        );
        loadRefunds();
      }
    } catch (err) {
  ...
  const handleExportCSV = () => {
    try {
      ...
      // Use the 'refunds' state which contains the data visible in your table
      const dataToExport = refunds;

      // 1. Check if the table actually has data
      if (!dataToExport || dataToExport.length === 0) {
        toast.error("No data in the table to export");
        return;
      }

      // 2. Extract Headers from the first object keys
      ...
      link.setAttribute(
        "download",
        `refund_ledger_${new Date().toISOString().split("T")[0]}.csv`,
      );
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-secondary/30 border-b border-border/50">
                <tr>
                  <th className="px-6 py-4 text-left font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">
                    Reference
                  </th>
                  <th className="px-6 py-4 text-left font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">
                    Customer
                  ...
                  <th className="px-6 py-4 text-right font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">
                    Amount
                  </th>
                  <th className="px-6 py-4 text-center font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">
                    Status
                  </th>
                  <th className="px-6 py-4 text-right font-semibold text-muted-foreground uppercase tracking-widest text-[10px]">
                    Actions
                ...
                {loading && pagination.page === 1 ? (
                  [1, 2, 3].map((i) => (
                    <tr key={i} className="animate-pulse">
                      <td
                        colSpan="6"
                        className="px-6 py-8 h-20 bg-muted/10"
                      ></td>
                    </tr>
                  ))
                ) : refunds.length === 0 ? (
                  ...
                  refunds.map((refund) => (
                    ...
                    >
                        ...
                        <div className="space-y-1">
                          <div className="font-mono text-[11px] font-bold text-primary flex items-center gap-1">
                            ...
                            {refund.return_id ? (
                              <Badge
                                variant="outline"
                                className="text-[9px] h-4 px-1 bg-purple-500/5 text-purple-600 border-purple-200"
                              >
                                Return
                              </Badge>
                            ) : (
                              <Badge
                                variant="outline"
                                className="text-[9px] h-4 px-1 bg-amber-500/5 text-amber-600 border-amber-200"
                              >
                                Cancel
                              </Badge>
                            )}
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="space-y-0.5">
                          ...
                          <p className="text-[10px] text-muted-foreground line-through opacity-50">
                            ₹{Number(refund.order_amount).toLocaleString()}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <Badge
                          className={cn(
                            "px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase border shadow-none",
                          ...
                          {refund.status === "settled" && (
                            <div className="flex flex-col items-end gap-1 opacity-50">
                              ...
                              <p className="text-[9px] text-muted-foreground">
                                {formatDate(refund.refund_settled_at)}
                              </p>
                            </div>
                          )}
                        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-12 mb-20">
        <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 flex gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Cancellations trigger immediate refunds. Returns trigger refunds
              only after the item reaches <b>RTO_COMPLETED</b> (Confirmed
              Warehouse Receipt).
            </p>
          </div>
        </div>
        <div className="p-6 rounded-3xl bg-secondary/20 border border-border/50 flex gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Customer bank details are shown here after being decrypted
              on-the-fly. They are stored encrypted (AES-256) in the core
